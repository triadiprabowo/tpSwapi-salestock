...
$script([
	...
	// Define angular module name
	window.tpSwapi = angular.module('tpSwapi', [
		'ngSanitize'
	]);
	...
	$script([
		...
		// Services
		'/dist/http/services/swapi.js',
		'/dist/http/services/bing.js',
		'/dist/http/directives/element.js'
	], function() {

		for(var i=0; i < appScript.controllers.length; i++) {
			appScript.controllers[i] = '/dist/http/controllers/'+appScript.controllers[i]+'.js'
		}

		// Load Controller
		$script(appScript.controllers, function() {
			// Set Bootstrap
			angular.bootstrap(document, ['tpSwapi']);
		});		
	});
});